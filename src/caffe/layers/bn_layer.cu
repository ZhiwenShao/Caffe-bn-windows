...
namespace caffe {
	...
	void BNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (use_global_stats_) {
			...
			caffe_copy(C_, this->blobs_[2]->gpu_data(), batch_mean_.mutable_gpu_data());
			caffe_copy(C_, this->blobs_[3]->gpu_data(), batch_variance_.mutable_gpu_data());
		}
		else{
			...
			caffe_copy(C_, batch_mean_.gpu_data(), this->blobs_[2]->mutable_gpu_data());
			caffe_copy(C_, batch_variance_.gpu_data(), this->blobs_[3]->mutable_gpu_data());
		}
