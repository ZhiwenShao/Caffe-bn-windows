...
namespace caffe {
	...
	void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
		...
		if (use_global_stats_) {
			...
			caffe_copy(C_, this->blobs_[2]->cpu_data(), batch_mean_.mutable_cpu_data());
			caffe_copy(C_, this->blobs_[3]->cpu_data(), batch_variance_.mutable_cpu_data());
		}
		else {
			...
			caffe_copy(C_, batch_mean_.cpu_data(), this->blobs_[2]->mutable_cpu_data());
			caffe_copy(C_, batch_variance_.cpu_data(), this->blobs_[3]->mutable_cpu_data());
		}
